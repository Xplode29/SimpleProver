...
let rec get_vars_from_arith = function
  ...
  |Add (a, b) | Mul (a, b) | Div (a, b) | Mod (a, b) | Pow (a, b) ->
    get_vars_from_arith a @ get_vars_from_arith b

let rec get_vars_from_formula = function
  |TrueF | FalseF -> []
  |NotF f -> get_vars_from_formula f
  |AndF (f1, f2) | OrF (f1, f2) | ImplyF (f1, f2) -> get_vars_from_formula f1 @ get_vars_from_formula f2
